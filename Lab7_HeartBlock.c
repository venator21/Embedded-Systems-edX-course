...
// manipulating DATA_R in a friendly way (only desired bits/pins of the port are manipulated) 
#define RED (*((volatile unsigned long *)0x40025008)) // Define bit 1 of DATA_R of port F (PF1) which is RED LED
#define GREEN (*((volatile unsigned long *)0x40025020)) // Define bit 3 of DATA_R of port F (PF3) which is GREEN LED
...
int main(void){
  ...
  while(1){          // Follows the nine steps list above
    ...
    WaitForASLow();        // b) wait for switch to be presseds
...
// write this function
	// START // Written by Piotr Brodzik
	while(SW1){ // loop while SW1 is not pressed
	}
	// END //
}
...
// write this function
	// START // Written by Piotr Brodzik
	while(!SW1){ // loop while SW1 is pressed
	}
	// END //
}
...
// write this function
  ...
  while(msec > 0 ) { // repeat while still halfsecs to delay
    count = 1333333/100;  // this number means 1msec
    while (count > 0) {
      count--;
    } 
    msec--;
