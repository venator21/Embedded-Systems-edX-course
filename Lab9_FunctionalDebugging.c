...
//START// Written by Piotr Brodzik
  while(1){
		if(!(GPIO_PORTF_DATA_R&0x01) || !(GPIO_PORTF_DATA_R&0x10)){ // check i SW1 or SW2 is pressed (either PF4 or PF0 is 0 - negative logic)
			Led = GPIO_PORTF_DATA_R;   // read previous
			Led = Led^0x02;            // toggle red LED
			GPIO_PORTF_DATA_R = Led;   // output
			current_IO = GPIO_PORTF_DATA_R&0x13;
			if((last_IO != current_IO) && i<50){ // dump data if either I/O changed and if empty room
				now = NVIC_ST_CURRENT_R;
				Time[i] = (last-now)&0x00FFFFFF;  // 24-bit time difference
				Data[i] = GPIO_PORTF_DATA_R&0x13; // record PF0, PF1, PF4 (LED, SW1, SW2)
				last = now;
				i++;
			}
			last_IO = current_IO;
